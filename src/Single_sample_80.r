# Side information based on barcode index and figure position (R version 4.3.2)
rm(list = ls())
source("../scr/R/spatial_fun_v1.r")
setwd("../doc/80")
barcode_index <- read.table("spatial_barcodes_index.txt", sep = "\t")
setwd("../data/80")
location <- read.table("position_54_hemibrain.txt", sep = ",", header = F, dec = ".", stringsAsFactors = F)
...
region <- 10000 #change the x axis maxium, need to adjust based on different sample
...
region <- 5000 #change the x axis maxium, need to adjust based on different sample
...
UMI_heatmap_plot <- umi_heatmap_80(test, count, color_manual, 10000)
...
Gene_heatmap_plot <- gene_heatmap_80(test, gene_count, color_manual, 5000)
ggsave(Gene_heatmap_plot, filename="Gene_count_heatmap.pdf", width = 8.6, height = 8.6)

# Cluster and specific genes ananlysis (R version 4.3.1)
rm(list = ls())
library(Seurat)
library(SeuratData)
library(ggplot2)
...
seurat_obj <- FindClusters(seurat_obj, resolution = 0.25, verbose = FALSE) 
umap_plot <- DimPlot(seurat_obj, label = TRUE, 
                     cols = c("#F6B78D", "#EF8943", "#D6DEBC", "#0D936B", "#C86018", 
                              "#D6D448", "#AC8768", "#D0DF64", "#FAEEAE", "#88CBC1", "#4C3279","grey")) +
  xlab("UMAP_1") + ylab("UMAP_2")  # the UMAP plot
ggsave(umap_plot, filename="Umap.pdf", width = 5.61, height = 4.53)

Vln_plot <- VlnPlot(seurat_obj, features = c("Prox1", "Calb1", "Wfs1", "Pcp4", "Grik4", "Dcx", "Slc17a7", "Gad1", "Pvalb", "Sst", "Npy", "Rgs14", "Nts", "Calb2",
                                             "Gpr6", "Cacna1g", "Tac1", "Slc6a1", "Ndnf", "Reln"), pt.size = 0.2, ncol = 2)
ggsave(Vln_plot, filename="VlnPlot.pdf", width = 5.17, height = 9.5)
Feature_plot <- FeaturePlot(seurat_obj, features = c("Prox1", "Calb1", "Wfs1", "Pcp4", "Grik4", "Dcx", "Slc17a7", "Gad1", "Pvalb", "Sst", "Npy", "Rgs14", "Nts", "Calb2",
                                                     "Gpr6", "Cacna1g", "Tac1", "Slc6a1", "Ndnf", "Reln"), pt.size = 0.2, ncol = 2)
...
# imported_raster = OpenImageR::readImage("white.jpg")
# g <- rasterGrob(imported_raster, width=unit(1,"npc"), height=unit(1,"npc"), interpolate = FALSE)
color_manual <- c("#F6B78D", "#EF8943", "#D6DEBC", "#0D936B", "#C86018", 
                  "#D6D448", "#AC8768", "#D0DF64", "#FAEEAE", "#88CBC1", "#4C3279","grey")
clusters_plot <- clusters_plot_80_blank(test, color_manual)
ggsave(clusters_plot, filename="Clusters.pdf", width = 6.89, height = 5.77)
...
seurat_obj.markers %>%
  ...
  slice_head(n = 5) %>%
  ungroup() -> top5
seurat_obj <- ScaleData(seurat_obj, features = top5$gene)
topexp_plot <- DoHeatmap(seurat_obj, features = top5$gene,
                         group.colors = c("#F6B78D", "#EF8943", "#D6DEBC", "#0D936B", "#C86018", 
                                          "#D6D448", "#AC8768", "#D0DF64", "#FAEEAE", "#88CBC1", "#4C3279","grey")) + NoLegend()
